...
namespace BookShopManagement.DAL
{
    class DAL_BookShop
    {
        ...
        {
            string query = "SELECT ID_Staff FROM Position AS p, Account AS a, Staff AS s WHERE(p.ID_Position = a.ID_Position) AND(a.ID_User = s.ID_User) AND p.NamePosition = 'Salesman'";
            return DBHelper.Instance.GetRecord1(query);
        }
    }
}
